...
<form class="form-inline booking-email-filter"
    ...
    <div class="form-group">
        ...
        <input type="number" min="0" value="<?php echo $from; ?>" name="from" class="form-control input-sm" id="out_of_from"
...
<form class="booking-email-form" action="" method="post">
    ...
    <table class="table table-bordered table-striped table-booking-history table-booking-history-email">
        ...
        <tr>
            ...
            <th><?php _e("Flight Info", ST_TEXTDOMAIN) ?></th>
            <th class="hidden-xs"><?php _e("Execution Time", ST_TEXTDOMAIN) ?></th>
        ...
        <?php if (!empty($posts)) {
            ...
            foreach ($posts as $key => $value) {
                ...
                <tr>
                    ...
                    <td class="">
                        <?php
                        $date = $value->check_in;
                        if ($date) echo esc_html__('Depart Date: ', ST_TEXTDOMAIN) . date('d/m/Y', strtotime($date)); ?>
                        <br>
                        <?php $date = $value->check_out;
                        if ($date) echo esc_html__('Return Date: ', ST_TEXTDOMAIN) . date('d/m/Y', strtotime($date)); ?>
                    </td>
            ...
            echo '<h5>' . __('No Tours', ST_TEXTDOMAIN) . '</h5>';
        }
        ?>
        </tbody>
    </table>
