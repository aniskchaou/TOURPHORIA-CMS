	      			...
	      			<div class="row">
	      				<div class="col-xs-12 col-sm-6">
	      					<div class="form-group">
						        <label for="hotel_id" class="head_bol"><?php _e('Room', ST_TEXTDOMAIN); ?> <span class="text-small text-danger">(*)</span>:</label>
						        <?php
			                        $room_id = isset($_POST['item_id']) ? $_POST['item_id'] : '';
			                        $room_name = (intval($room_id) > 0) ? get_the_title($room_id) : "";
			                    ?>
			                    <input id="room_id" type="hidden" name="item_id" value="<?php echo $room_id; ?>" data-post-type="hotel_room" class="form-control custom-form-control st_post_select_ajax " data-pl-name="<?php echo $room_name; ?>" data-user-id="<?php echo get_current_user_id(); ?>">
