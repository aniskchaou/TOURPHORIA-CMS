...
<div class="col-lg-9 col-md-9">
    <?php echo st()->load_template('layouts/modern/hotel/elements/toolbar', '', array('style' => $style, 'format' => $format, 'layout' => $layout, 'service_text' => __('New car', ST_TEXTDOMAIN), 'post_type' => 'st_cars')); ?>
    <div id="modern-search-result" class="modern-search-result" data-layout="1">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/modern/car/elements/loop/' . $style);
            }
        }else{
    ...
    <div class="pagination moderm-pagination" id="moderm-pagination" data-layout="normal">
        ...
        <span class="count-string">
            ...
            if ($query->found_posts):
                ...
                echo sprintf(__('%d - %d of %d %s', ST_TEXTDOMAIN), (int)$posts_per_page * ((int)$page - 1) + 1, $last, $query->found_posts, ($query->found_posts == 1 ? 'Car' : 'Cars'));
            endif;
            ?>
        </span>
    </div>
