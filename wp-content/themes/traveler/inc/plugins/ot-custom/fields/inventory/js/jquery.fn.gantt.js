...
(function ($) {
    ...
    $.fn.gantt = function (options) {
        ...
        var core = {
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '" data-id="' + entry.id + '">' + (entry.name + '<div class="inventory-edit-room-number dashicons dashicons-edit"></div>' || '') + '</span>');
                        entries.push('</div>');
                ...
                dataPanel.click(function (e) {
                    ...
                    if (core.clickStart && core.flagClick && core.clickEnd == null) {
                        ...
                        if ($('.panel-price-wrapper', element).length == 0) {
                            $(element).append('<div class="panel-price-wrapper"><div class="panel-price"><input class="input-price" type="text" name="input-price" value="" placeholder="Price"><select class="input-status" name="input-status"><option value="available">Available</option><option value="unavailable">Not Available</option></select><a href="javascript: void(0);" class="button wb-btn-add-price" style="margin-left: 10px;">Update <i class="fa fa-spin fa-spinner loading-icon"></i></a><span class="close"><i class="fa fa-times"></i></span></div></div>');
                                    ...
                                    .click(function () {
                                        if ($('.inventory-guide', element).length == 0) {
                                            $(element).append('<div class="inventory-guide-wrapper"><div class="inventoty-guide">' +
                                                '<h3>How to use?</h3>' +
                                                '<p><strong>Change price/availability:</strong> Click on <strong>start date</strong> then hover to <strong>end date</strong> and click again. Enter the price in the text field when the popup is displayed</p>' +
                                                '<p><strong>Change the display date range</strong>: Click on the icons at the bottom (today, previous month, next month) or enter the date in <strong>the start date, end date</strong> field and click <strong>the View button</strong>.</p>' +
                                                '<p><strong>Change the number of room</strong>: Click on the pen icon near the name of room. Enter the number in the text field when the popup is displayed</p>' +
            ...
            // the entire chart
            createProgressBar: function (id, days, cls, desc, label, dataObj, price) {
                var cellWidth = tools.getCellSize();
                var barMarg   = tools.getProgressBarMargin() || 0;
                var bar       = $('<div class="bar"><div class="fn-label">' + label + '<div class="price">' + price + '</div>' + '</div></div>')
                    .addClass(cls)
                    .css({
