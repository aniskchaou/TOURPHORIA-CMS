                    ...
                    if ( !empty( $check_in ) && !empty( $check_out ) ) {
                        ...
                        if ( !empty( $arr_to_insert ) ) {
                            foreach ( $arr_to_insert as $kk => $vv ) {
                                $string_insert .= $wpdb->prepare( "(null, %s, %s, %d, %d, %d, %s, %d, %d, %s, %s,%s, %s, %s),", 'hotel_room', '0', $number, $parent_id, $booking_period, $allow_full_day, $adult_number, $child_number, $base_id, $vv, $vv, $price, 'available' );
                            }
                        }
                    }
                } else {
                    for ( $i = $check_in; $i <= $check_out; $i = strtotime( '+1 day', $i ) ) {
                        $string_insert .= $wpdb->prepare( "(null, %s, %s, %d, %d, %d, %s, %d, %d, %s, %s,%s, %s, %s),", 'hotel_room', '0', $number, $parent_id, $booking_period, $allow_full_day, $adult_number, $child_number, $base_id, $i, $i, $price, 'available' );
                    }
                }
...
//				) ");
            ...
            public function _change_posts_fields( $fields )
            {
                ...
                $fields .= ', min(CAST(tb.price AS DECIMAL) - (CAST(tb.price AS DECIMAL) * ((IFNULL(tb3.discount_rate, 1))/100))) as st_price, tb2.price_avg as st_price_avg';
