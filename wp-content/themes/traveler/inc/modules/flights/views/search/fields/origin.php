...
<div class="form-group form-group-<?php echo esc_attr($field_size)?> form-group-icon-left flight-input">
    ...
    <div class="st-select-wrapper">
        <input  autocomplete="off" type="text" name="origin_name" value="<?php echo esc_attr($origin_name); ?>" class="form-control st-flight-location-name <?php echo esc_attr($is_required); ?>" placeholder="<?php if($placeholder) echo $placeholder; ?>">
        <select  name="origin" class="st-flight-location-id st-hidden" placeholder="<?php if($placeholder) echo $placeholder; ?>" tabindex="-1">
            <option value=""></option>
            <?php
