...
class TravelerObject
{
     ...
     *
     * */
    function get_near_by($post_id = false, $range = 5000, $limit = 4)
    {
        if (!$post_id)
            $post_id = get_the_ID();
        // Remove in ver 2.0
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
    ...
    function _show_wc_cart_item_information_btn($cart_item_key = [])
    {
        ...
        print balancetags('<br><span data-hide = "' . __("Less", ST_TEXTDOMAIN) . ' <i class=&quot;fa fa-angle-up&quot;>" data-more = "' . __("More", ST_TEXTDOMAIN) . ' <i class=&quot;fa fa-angle-down&quot;>" data-target= "#st_cart_item' . md5(json_encode($cart_item_key)) . '" data-toggle="collapse" class="_show_wc_cart_item_information_btn text-color booking-item-review-expand-new">' . __("More", ST_TEXTDOMAIN) . ' <i class="fa fa-angle-down"></i></span>');
    }
