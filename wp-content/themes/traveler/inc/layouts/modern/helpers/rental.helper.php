...
if (!class_exists('RentalHelper')) {
    class RentalHelper
    {
        ...
        static function _get_availability_rental_single_groupday()
        {
            ...
            $data_rental = AvailabilityHelper::_getdataRentalEachDate($rental_origin, strtotime($check_in), strtotime($check_out));
                            ...
                            if ($period_in_out > 0) {
                                ...
                                if (in_array($d, $list_date)) {
                                    $events['events'][] = [
                                        ...
                                        'event' => esc_attr__('Booked', ST_TEXTDOMAIN),
                                    ...
                                    if ($val->check_in < $today) {
                                        $events['events'][] = [
                                            'start' => date('Y-m-d', $val->check_in),
                                            'end' => date('Y-m-d', $val->check_out),
                                            'event' => esc_attr__('Unavailable', ST_TEXTDOMAIN),
                                            'status' => 'not_available',
                                        ];
                                    } else {
                                        // disabled
